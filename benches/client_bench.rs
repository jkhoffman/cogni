...
use cogni::{Error, Message, Provider, Request, Response, StreamEvent};
use cogni_client::{parallel_requests, Client};
    ...
    c.bench_function("parallel_requests_3_providers", |b| {
        b.iter(|| {
            rt.block_on(async {
                let providers = vec![
                    MockProvider {
                        delay: Some(tokio::time::Duration::from_micros(10)),
                    },
                    MockProvider {
                        delay: Some(tokio::time::Duration::from_micros(10)),
                    },
                ...
                let request = Request::builder().message(Message::user("Test")).build();
