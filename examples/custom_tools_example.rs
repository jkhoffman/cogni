...
use cogni_tools::{ToolRegistry, FunctionExecutorBuilder, ToolExecutor, ToolCall, validation::param_schema};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for tool in registry.list_tools().await {
        ...
        println!("  Parameters: {}", serde_json::to_string_pretty(&tool.function.parameters)?);
        println!();
    }
