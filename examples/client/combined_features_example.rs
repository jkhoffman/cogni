...
use cogni_core::{Message, Provider, Tool, ToolFunction};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let (Ok(openai_key), Ok(anthropic_key)) =
        (env::var("OPENAI_API_KEY"), env::var("ANTHROPIC_API_KEY"))
    {
...
async fn compare_providers(
    ...
    let (openai_result, anthropic_result) =
        tokio::join!(openai.request(request.clone()), anthropic.request(request));
...
async fn streaming_with_state(api_key: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  Tokens/sec: {:.2}",
        state.tokens as f64 / elapsed.as_secs_f64()
    );
