...
use cogni::tools::{executor::FunctionExecutorBuilder, ToolExecutor, ToolRegistry};
...
fn create_weather_tool() -> Box<dyn ToolExecutor> {
    Box::new(
...
fn create_calculator_tool() -> Box<dyn ToolExecutor> {
    Box::new(
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Method 1: Create registry using from_executors
    ...
    registry2.register_boxed(create_weather_tool()).await?;
    registry2.register_boxed(create_calculator_tool()).await?;
