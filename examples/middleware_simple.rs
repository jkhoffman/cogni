...
use cogni_core::{Provider, Request, Message, Response, Error};
...
impl<P: Provider> Provider for LoggingProvider<P> {
    ...
    fn request(&self, request: Request) -> impl Future<Output = Result<Response, Error>> + Send {
        async move {
            println!("📝 Request: {} messages to {}", request.messages.len(), request.model.0);
...
async fn main() -> Result<(), Error> {
    ...
    let provider = TimingProvider::new(
        LoggingProvider::new(openai)
    );
