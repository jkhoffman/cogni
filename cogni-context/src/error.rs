...
mod tests {
    ...
    fn test_error_display() {
        ...
        assert_eq!(err.to_string(), "Token counting error: Failed to initialize");

        let err = ContextError::ContextExceeded {
            current: 5000,
            max: 4096,
        };
        assert_eq!(err.to_string(), "Context exceeded: 5000 tokens > 4096 tokens");
    ...
    fn test_all_error_variants_convert() {
        let errors = vec![
            ...
            ContextError::ContextExceeded { current: 100, max: 50 },
