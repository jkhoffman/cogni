...
use cogni::prelude::*;
use cogni::{Tool, ToolCall, Function, RequestBuilder};
use cogni::providers::{OpenAI, Anthropic, Ollama};
use cogni::tools::{ToolRegistry, builtin};
...
async fn test_openai_tool_execution() {
    ...
    let mut builder = Request::builder()
        .model("gpt-4")
        .message(Message::user("What is 25 * 4? Please use the calculator tool."))
        .max_tokens(150);
    ...
    assert!(!response.tool_calls.is_empty(), "Expected tool calls but got none");
...
async fn test_anthropic_tool_execution() {
    ...
    let mut builder = Request::builder()
        ...
        .message(Message::user("What is 15 + 27? Use the calculator tool to compute this."))
        .max_tokens(150);
...
async fn test_ollama_tool_execution() {
    ...
    let mut builder = Request::builder()
        ...
        .message(Message::user("Calculate 12 divided by 3 using the calculator tool."));
